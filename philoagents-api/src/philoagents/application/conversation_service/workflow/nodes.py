...
from philoagents.application.conversation_service.workflow.chains import (get_context_summary_chain,
...
async def conversation_node(state: ConversationState, config: RunnableConfig):
 ...
 """
    ...
    response = await conversation_chain.ainvoke(
        {"messages": state["messages"], "retrieved_context": state.get("retrieved_context", ""),
         "character_name": state["character_name"], "character_perspective": state["character_perspective"],
         "character_style": state["character_style"], "summary": summary, }, config, )
...
async def summarize_conversation_node(state: ConversationState):
    ...
    response = await summary_chain.ainvoke(
        {"messages": state["messages"], "character_name": state["character_name"], "summary": summary, })
...
async def summarize_context_node(state: ConversationState):
   ...
   """
    ...
    response = await context_summary_chain.ainvoke({"context": tool_output_message.content, })
