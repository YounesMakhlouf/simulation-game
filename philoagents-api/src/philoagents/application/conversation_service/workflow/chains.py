...
from philoagents.domain.prompts import (CONTEXT_SUMMARY_PROMPT, EXTEND_SUMMARY_PROMPT, SUMMARY_PROMPT,
...
def get_character_response_chain():
    ...
    system_message = DELEGATE_CONVERSATIONAL_PROMPT

    prompt = ChatPromptTemplate.from_messages(
        [("system", system_message.prompt), MessagesPlaceholder(variable_name="messages"), ],
...
def get_conversation_summary_chain(summary: str = ""):
    ...
    prompt = ChatPromptTemplate.from_messages(
        [MessagesPlaceholder(variable_name="messages"), ("human", summary_message.prompt), ],
        template_format="jinja2", )
