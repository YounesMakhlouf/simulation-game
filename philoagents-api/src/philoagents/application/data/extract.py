...
class RagExtractor:
    ...
    def get_extraction_generator(self, rag_sources: List[Dict]) -> Generator[
        ...
        """
        progress_bar = tqdm(rag_sources, desc="Extracting RAG docs", unit="character",
    ...
    def _extract_for_character(self, character: Character, source_info: Dict) -> List[Document]:
        """
        ...
        if "wikipedia_query" in source_info:
            docs.extend(self._extract_wikipedia(character, source_info["wikipedia_query"]))

        if "britannica_urls" in source_info:
            docs.extend(self._extract_britannica(character, source_info["britannica_urls"]))
    ...
    def _extract_britannica(character: Character, urls: List[str]) -> List[Document]:
        ...
        def clean_britannica_html(soup: BeautifulSoup) -> str:
            excluded_selectors = ["aside#toc", ".md-quick-fact", ".md-header-tools", "figcaption", ".md-related-links",
                                  ".md-grid-group", "footer"]
            for selector in excluded_selectors:
                for element in soup.select(selector):
                    element.decompose()
            main_article = soup.find("article")
            if not main_article: return ""
            return "\n\n".join(element.get_text() for element in main_article.find_all(["p", "h1", "h2", "h3", "h4"]))
...
if __name__ == "__main__":
    ...
    extraction_generator = extractor.get_extraction_generator(rag_sources=rag_sources_data)
