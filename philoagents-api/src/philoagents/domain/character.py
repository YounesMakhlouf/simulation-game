...
class CharacterExtract(BaseModel):
    ...
    id: str = Field(
        description="A unique, machine-readable identifier for the character (e.g., 'metternich')."
    )
    urls: List[str] = Field(
...
class Character(BaseModel):
    ...
    id: str = Field(
        description="Unique identifier for the character (e.g., 'metternich')."
    )
    name: str = Field(
        description="The full name of the historical figure (e.g., 'Klemens von Metternich')."
    )
    perspective: str = Field(
        description="A description of the character's core beliefs, political stance, and worldview."
    )
    style: str = Field(
        description="A description of the character's conversational and diplomatic style."
    )
    goals: str = Field(
        description="The primary objectives this character aims to achieve during the simulation."
    )
    resources: Dict[str, int] = Field(
        description="A dictionary of the character's starting resources (e.g., {'DiplomaticInfluence': 100, 'Spies': 5})."
    )
    known_intel: List[str] = Field(
